...
def get_version_major_minor():
    return tuple([int(n) for n in get_version().split(".", 2)])
    ...
    result.extend(
        [
            dict(
                ...
                url="https://prdownloads.sourceforge.net/tcl/tcl%s-src.tar.gz"
                % (tcl_tk_ver,),
                ...
                % {
                    "DESTDIR": shell_quote(os.path.join(WORKDIR, "libraries")),
                    "TCL_LIBRARY": shell_quote("%s/lib/tcl8.6" % OUTDIR),
                },
            ),
            dict(
                name="Tk %s" % (tcl_tk_ver,),
                url="https://prdownloads.sourceforge.net/tcl/tk%s-src.tar.gz"
                % (tcl_tk_ver,),
                ...
                % {
                    "DESTDIR": shell_quote(os.path.join(WORKDIR, "libraries")),
                    "TCL_LIBRARY": shell_quote("%s/lib/tcl8.6" % OUTDIR),
                    "TK_LIBRARY": shell_quote("%s/lib/tk8.6" % OUTDIR),
                },
            ),
        ]
    )
    ...
    result.extend(
        [
            dict(
                ...
                % (shell_quote(os.path.join(WORKDIR, "libraries")),),
            ),
            dict(
                name="SQLite 3.37.2",
...
def capture_command(commandline):
    fd = os.popen(commandline, "r")
    data = fd.read()
    xit = fd.close()
    if xit is not None:
...
def parse_options(args=None):
    ...
    print("")
    print(
        " -- Building a Python %s embed at patch level %s"
        % (get_version(), get_full_version())
    )
    print("")


def extract_archive(builddir, archiveName):
    """
    ...
    try:
        ...
        if archiveName.endswith(".tar.gz"):
            ...
            fp = os.popen("tar zxf %s 2>&1" % (shell_quote(archiveName),), "r")

        elif archiveName.endswith(".tar.bz2"):
            retval = os.path.basename(archiveName[:-8])
            if os.path.exists(retval):
                shutil.rmtree(retval)
            fp = os.popen("tar jxf %s 2>&1" % (shell_quote(archiveName),), "r")

        elif archiveName.endswith(".tar"):
            retval = os.path.basename(archiveName[:-4])
            if os.path.exists(retval):
                shutil.rmtree(retval)
            fp = os.popen("tar xf %s 2>&1" % (shell_quote(archiveName),), "r")

        elif archiveName.endswith(".zip"):
            retval = os.path.basename(archiveName[:-4])
            if os.path.exists(retval):
                shutil.rmtree(retval)
            fp = os.popen("unzip %s 2>&1" % (shell_quote(archiveName),), "r")
...
def verify_third_party_file(url, checksum, fname):
    """
...
def build_universal_openssl(basedir, archList):
    ...
    def build_openssl_arch(archbase, arch):
        ...
        run_command(" ".join(["perl", "Configure"] + arch_opts[arch] + configure_opts))
        run_command("make depend")
        run_command("make all")
        run_command("make install_sw DESTDIR=%s" % shell_quote(archbase))
    ...
    shlib_version_number = grep_value(
    ...
    try:
        os.mkdir(os.path.join(base, *OUTDIR.split("/"), "lib"))
    except OSError:
        pass
    ...
    for (lib_unversioned, lib_versioned) in [
        ...
        run_command("lipo -create -output "
                    + " ".join(
            shell_quote(os.path.join(fw, *OUTDIR.split("/"), "lib", lib_versioned))
            for fw in archbases
        ))
        # and create an unversioned symlink of it
        os.symlink(
            lib_versioned,
            os.path.join(base, *OUTDIR.split("/"), "lib", lib_unversioned),
        )
...
def build_recipe(recipe, basedir, archList):
    """
    ...
    install = recipe.get(
        "install", "make && make install DESTDIR=%s" % (shell_quote(basedir))
    )
    ...
    verify_third_party_file(url, recipe["checksum"], sourceArchive)
    print("Extracting archive for %s" % (name,))
    buildDir = os.path.join(WORKDIR, "_bld")
    if not os.path.exists(buildDir):
        os.mkdir(buildDir)

    workDir = extract_archive(buildDir, sourceArchive)
    os.chdir(workDir)
    ...
    for patchscript in recipe.get("patchscripts", ()):
        ...
        if fn.endswith(".bz2"):
            run_command("bunzip2 -fk %s" % shell_quote(fn))
            fn = fn[:-4]
        run_command("sh %s" % shell_quote(fn))
        os.unlink(fn)
    ...
    if configure is not None:
            ...
            configure_args.extend(
                [
                    ...
                    % (
                        ...
                        DEPTARGET,
                        " -arch ".join(archList),
                        shell_quote(basedir)[1:-1],
                    ),
                    "LDFLAGS=-mmacosx-version-min=%s -L%s/usr/local/lib -arch %s"
                    % (DEPTARGET, shell_quote(basedir)[1:-1], " -arch ".join(archList)),
                ]
            )
        else:
            configure_args.extend(
        ...
        configure_args = [shell_quote(a) for a in configure_args]

        print("Running configure for %s" % (name,))
        run_command(" ".join(configure_args) + " 2>&1")
...
def build_libraries():
    ...
    for recipe in library_recipes():
        build_recipe(recipe, universal, ARCHLIST)
    libraries_postbuild()
...
def build_python():
    ...
    run_command("%s -C --enable-universalsdk=/ "
                ...
                % (
                    shell_quote(os.path.join(SRCDIR, "configure")),
                    UNIVERSALARCHS,
                    rootDir,
                    shell_quote(WORKDIR)[1:-1],
                    shell_quote(WORKDIR)[1:-1],
                    shell_quote(WORKDIR)[1:-1],
                    shell_quote(WORKDIR)[1:-1],
                    shell_quote(WORKDIR)[1:-1],
    ...
    runshared_for_make = "".join(
        [" RUNSHARED=", "'", grep_value("Makefile", "RUNSHARED"), "'"]
    )
...
def python_postbuild():
    ...
    path_to_lib = os.path.join(embed_lib_dir, "python%s" % (get_version()))
    ...
    for dirpath, dirnames, filenames in os.walk(OUTDIR):
        ...
        for fn in filenames:
            ...
            if fn in EXPECTED_SHARED_LIBS:
                ...
                data = capture_command("otool -L %s" % shell_quote(p))
    ...
    if get_version_major_minor() >= (3, 6):
